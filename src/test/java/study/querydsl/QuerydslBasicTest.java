...
//        QMember m = QMember.member;
    ...
    public void searchAndParam() {
        Member findMember = queryFactory
                ...
                .where(
                        ...
                        member.age.eq(10) // 위의 search() 테스트와 동일 and 조건 sql 쿼리
                )
                .fetchOne();
    ...
    public void aggregation() {
        ...
        Tuple tuple = result.get(0); // 실무에서는 Tuple보다는 Dto 사용
...
//            System.out.println("-> tuple.get(team)=" + tuple.get(team));
    ...
    public void fetchJoinUse() {
        ...
        Member findMember = queryFactory
                .selectFrom(member)
                .join(member.team, team).fetchJoin() // fetchJoin(): 페치 조인
                .where(member.username.eq("member1"))
                .fetchOne();
    ...
    @Test
    public void tupleProjection() { // Tuple 타입의 변수는 repository 계층 안에서만 사용하는 것을 지향
        List<Tuple> result = queryFactory
...
//                .where(usernameEq(usernameCond), ageEq(ageCond))
     ...
     * -> flush(), clear() 사용
     * bulk 연산 실행 후에는 영속성 컨텍스트를 초기화 해주는 것이 좋다.
     */
    @Test
...
//                        Expressions.stringTemplate("function('lower', {0})", member.username)))
                .where(member.username.eq(member.username.lower())) // 웬만한 기본 sql function은 QueryDSL에 내장되어 있다.
                .fetch();
